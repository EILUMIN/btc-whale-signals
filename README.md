...
- Each whale wall card shows **APPROACHING / DISTANT / HIT / REJECTED / BROKEN / REMOVED**. APPROACHING is only when the wall is fresh and within `WALL_APPROACHING_PCT` (1.5%) of the live snapshot price. Farther walls are DISTANT and are not treated as nearby support or resistance. HIT is only when live price is inside that card's displayed range. HIT never fires LONG/SHORT by itself. A wall not seen for `WALL_STALE_SEC` (120) is REMOVED.
...
- **Overall Market Bias** is a third live-candle field (H1 direction, M15 structure, M5 confirm, M1 timing). It never replaces the whale tape or Possible Entry. Conflict across those timeframes stays WAIT.
